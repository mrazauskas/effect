 ...
 */
export const primaryKey = <R extends Rpc.Any>(envelope: Envelope<R>): string => {
  if (envelope._tag !== "Request") {
    return envelope.id.toString()
  }
  return Predicate.hasProperty(envelope.payload, PrimaryKey.symbol)
    ? PrimaryKey.value(envelope.payload)
