import { MessageStorage, Pods, Sharding, ShardingConfig, ShardManager, Snowflake } from "@effect/cluster"
import { assert, describe, expect, it } from "@effect/vitest"
import { Array, Cause, Chunk, Effect, Exit, Fiber, Layer, Mailbox, Option, Stream, TestClock } from "effect"
  ...
  it.scoped("interrupts are sent for volatile messages on shutdown", () =>
    Effect.gen(function*() {
      ...
      yield* Effect.gen(function*() {
        ...
        yield* TestClock.adjust(1)
        const client = makeClient("1")
        yield* client.NeverVolatile().pipe(Effect.fork)
        yield* TestClock.adjust(1)
      }).pipe(Effect.provide(TestShardingWithoutPods.pipe(
