import { Entity, MessageStorage, PodAddress, ShardingConfig, Singleton } from "@effect/cluster"
...
for (let i = 0; i < 10; i++) {
  const ShardingLive = NodeClusterSocketPods.layer.pipe(
    Layer.provide([
      RpcSerialization.layerMsgPack,
      ShardingConfig.layer({
        podAddress: Option.some(PodAddress.make("localhost", 50000 + i))
      }),
