...
describe("sql", () => {
    it.effect("tranformers", () => Effect.gen(function* () {
        const logs = [];
        const sql = yield* SqlClient.SqlClient;
        const result = yield* sql `SELECT 1 + 1 AS result`.pipe(Effect.provide(Logger.replace(Logger.defaultLogger, Logger.make((log) => {
            logs.push(log.message);
        }))));
        assert.deepStrictEqual(result, [{ result: 2 }]);
        assert.deepStrictEqual(logs, [["executing sql"]]);
    }).pipe(Effect.provide(MysqlContainer.ClientLive.pipe(Layer.provide(SqlLogger))), Effect.catchTag("ContainerError", () => Effect.void)), { timeout: 60_000 });
});
