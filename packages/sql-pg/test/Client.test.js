...
it.layer(PgContainer.ClientLive, { timeout: "30 seconds" })("PgClient", (it) => {
    it.effect("insert helper", () => Effect.gen(function* () {
    ...
    it.effect("updateValues helper", () => Effect.gen(function* () {
        const sql = yield* PgClient.PgClient;
        const [query, params] = sql `UPDATE people SET name = data.name FROM ${sql.updateValues([{ name: "Tim" }, { name: "John" }], "data")}`.compile();
        expect(query).toEqual(`UPDATE people SET name = data.name FROM (values ($1),($2)) AS data("name")`);
        expect(params).toEqual(["Tim", "John"]);
    }));
    it.effect("updateValues helper returning", () => Effect.gen(function* () {
        const sql = yield* PgClient.PgClient;
        const [query, params] = sql `UPDATE people SET name = data.name FROM ${sql.updateValues([{ name: "Tim" }, { name: "John" }], "data").returning("*")}`.compile();
        expect(query).toEqual(`UPDATE people SET name = data.name FROM (values ($1),($2)) AS data("name") RETURNING *`);
        expect(params).toEqual(["Tim", "John"]);
    }));
    it.effect("update helper", () => Effect.gen(function* () {
        const sql = yield* PgClient.PgClient;
    ...
    it.effect("update helper returning", () => Effect.gen(function* () {
        const sql = yield* PgClient.PgClient;
    ...
    it.effect("array helper", () => Effect.gen(function* () {
        const sql = yield* PgClient.PgClient;
    ...
    it.effect("array helper with column", () => Effect.gen(function* () {
        const sql = yield* PgClient.PgClient;
    ...
    it.effect("and", () => Effect.gen(function* () {
        const sql = yield* PgClient.PgClient;
        const now = new Date();
    ...
    it.effect("json", () => Effect.gen(function* () {
        const sql = yield* PgClient.PgClient;
        const [query, params] = sql `SELECT ${sql.json({ a: 1 })}`.compile();
        expect(query).toEqual(`SELECT $1`);
        expect(params[0].type).toEqual(3802);
    }));
    it.effect("json transform", () => Effect.gen(function* () {
        const sql = yield* PgClient.PgClient;
        const [query, params] = compilerTransform.compile(sql `SELECT ${sql.json({ aKey: 1 })}`, false);
        expect(query).toEqual(`SELECT $1`);
        assert.deepEqual(params[0].value, { a_key: 1 });
    }));
    it.effect("array", () => Effect.gen(function* () {
        const sql = yield* PgClient.PgClient;
        const [query, params] = sql `SELECT ${sql.array([1, 2, 3])}`.compile();
        expect(query).toEqual(`SELECT $1`);
        expect(params[0].value).toEqual([1, 2, 3]);
    }));
    it("transform nested", () => {
        assert.deepEqual(transformsNested.array([
            {
                a_key: 1,
                nested: [{ b_key: 2 }],
    ...
    it("transform non nested", () => {
        assert.deepEqual(transforms.array([
            {
                a_key: 1,
                nested: [{ b_key: 2 }],
                arr_primitive: [1, "2", true]
    ...
    it.effect("insert fragments", () => Effect.gen(function* () {
        const sql = yield* PgClient.PgClient;
        ...
        expect(params[2].type).toEqual(3802);
    }));
    it.effect("insert array", () => Effect.gen(function* () {
        const sql = yield* PgClient.PgClient;
        const [query, params] = sql `INSERT INTO people ${sql.insert({
            name: "Tim",
            age: 10,
        ...
        assert.lengthOf(params, 3);
        expect(params[2].type).toEqual(1022);
    }));
    it.effect("update fragments", () => Effect.gen(function* () {
        const sql = yield* PgClient.PgClient;
        const now = new Date();
        const [query, params] = sql `UPDATE people SET json = data.json FROM ${sql.updateValues([{ json: sql.json({ a: 1 }) }, { json: sql.json({ b: 1 }) }], "data")} WHERE created_at > ${now}`.compile();
        assert.strictEqual(query, `UPDATE people SET json = data.json FROM (values ($1),($2)) AS data("json") WHERE created_at > $3`);
        assert.lengthOf(params, 3);
        expect(params[0].type).toEqual(3802);
        expect(params[1].type).toEqual(3802);
    }));
    it.effect("onDialect", () => Effect.gen(function* () {
        const sql = yield* PgClient.PgClient;
        assert.strictEqual(sql.onDialect({
            sqlite: () => "A",
            pg: () => "B",
            mysql: () => "C",
            mssql: () => "D",
        ...
        assert.strictEqual(sql.onDialectOrElse({
            orElse: () => "A",
            pg: () => "B"
        }), "B");
    }));
    it.effect("identifier transform", () => Effect.gen(function* () {
        const sql = yield* PgClient.PgClient;
...
it.layer(PgContainer.ClientTransformLive, { timeout: "30 seconds" })("PgClient transforms", (it) => {
    ...
    it.effect("insert helper withoutTransforms", () => Effect.gen(function* () {
